...
| P2 / P3 | 🔧 / 🔜 | **P2**（进行中）：A7C2 USB 直连 —— 设计稿 `docs/P2_DESIGN.md`；**PoC-1/2/3 已落地**：① 免权限 USB 枚举 + Sony VID/接口类识别（`camera/CameraProbe`、`camera/UsbCameraScanner`）；② USB 授权 + 打开设备 + 声明 PTP 接口（`camera/CameraConnection`）；③ PTP 会话握手 + 设备信息 + 存储/对象枚举（`camera/PtpProtocol` 容器编解码、`camera/PtpData` 数据集解析、`camera/PtpTransport` Bulk 传输、`camera/CameraPtpReport` 报告），首页 `CameraPanel` 一键跑完并写 `CAMERA` 日志；协议层与数据集解析均有纯 JVM 单测。后续 PoC-4/5：`GetObject` 流式下载单张 ARW → **复用 P1 管线套预设**（`Presets.ALL` 已就绪）→ 批量队列。**P3**：NAS Docker 化 Rust 引擎。 |
