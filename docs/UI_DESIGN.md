...
| 层 | 高度 | 内容 |
|---|---|---|
| ① 顶栏 | 56dp | 返回 / 撤销 / 重做 / 重置 / **导出**（导出为玻璃胶囊，其余为 ghost 文字按钮） |
| ② 预览区 | `weight(previewFraction)`，默认 0.62 | 照片（**按 `fitContentRect` 定尺，无黑边**）+ 按住对比 + 蒙版笔刷/祛瑕落点 + 胶囊提示 |
| ③ **分隔条** | 28dp 热区 / 4dp 视觉线 | 上下拖动即改「预览 : 工具区」高度比（`SPLIT_MIN 0.28` ~ `SPLIT_MAX 0.78`） |
...
| # | 问题 | 根因 | 修法 |
|---|---|---|---|
| 1 | 拖滑块时画面会跳两下 | 顶栏走 `AnimatedVisibility` **收高度**，而预览区高度是 `weight(previewFraction)` ⇒ 分母变大 56dp、照片跟着放大，松手又缩回 | 顶栏改**只降 `alpha`、不收高度**，与工具条 / 分隔条统一口径：拖动期间**布局完全静止**，只有透明度在变 |
